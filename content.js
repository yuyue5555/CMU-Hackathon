  ...
  window.PositiveContentReplacer = class PositiveContentReplacer {
    ...
    async init() {
      ...
      const result = await chrome.storage.sync.get([
        'isEnabled', 'showHighlight', 'useAI', 'aiService', 'systemPromptKey', 'customWords'
      ]);
    ...
    containsNegativeKeywords(text) {
      ...
      for (const wordPair of this.customWords) {
        if (lowerText.includes(wordPair.negative.toLowerCase())) {
          return true;
        }
      }
      return false;
     ...
     * @returns {Object} - { text: processed text, hasReplacements: boolean }
     */
    applyCustomWordReplacements(text) {
      let processedText = text;
      let hasReplacements = false;

      // Apply custom word replacements first (higher priority than AI)
      for (const wordPair of this.customWords) {
        const regex = new RegExp(`\\b${this.escapeRegex(wordPair.negative)}\\b`, 'gi');
        const newText = processedText.replace(regex, wordPair.positive);
      ...
      return { text: processedText, hasReplacements };
    }
    ...
    async replaceWithAI(textNode, originalText) {
      try {
        // 再次检查文本节点是否仍然有效
        ...
        // 步骤 1: 先应用自定义词汇替换（优先级最高）
        const customReplacement = this.applyCustomWordReplacements(originalText);
        let textToProcess = customReplacement.text;
        const hasCustomReplacements = customReplacement.hasReplacements;

        console.log('自定义词汇替换:', {
          original: originalText,
          afterCustom: textToProcess,
          hasReplacements: hasCustomReplacements
        });

        // 如果只有自定义替换且文本已经改变，可以选择是否继续AI处理
        // 这里我们选择：如果有自定义替换但仍包含其他负面词，则继续AI处理
        const stillHasNegativeWords = this.containsNegativeKeywords(textToProcess);

        let finalText = textToProcess;
        let service = 'custom';

        // 步骤 2: 如果仍有负面词或没有自定义替换，则使用 AI 进一步处理
        if (stillHasNegativeWords || !hasCustomReplacements) {
          console.log('开始 AI 转换:', textToProcess);
        ...
        // 只有当最终文本与原始文本不同时才进行替换
        if (finalText !== originalText && finalText.length > 0) {
          console.log('最终转换:', originalText, '→', finalText);
    ...
    async updateSettings(settings) {
      ...
      console.log('更新设置，自定义词汇:', this.customWords);
